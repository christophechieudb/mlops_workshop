...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=data-science&org_id=1444828305810485&notebook=%2F01-mlops-quickstart%2F02_automl_best_run&demo_name=mlops-end2end&event=VIEW&path=%2F_dbdemos%2Fdata-science%2Fmlops-end2end%2F01-mlops-quickstart%2F02_automl_best_run&version=1">

# COMMAND ----------

# DBTITLE 1,Install MLflow version for UC [for MLR < 15.2]
# MAGIC %pip install --quiet databricks-sdk==0.23.0 mlflow==2.14.0 databricks-automl-runtime holidays category-encoders lightgbm hyperopt shap
...
#Let's get some information about the different experiments happened when we launched AutoML in 01_feature_engineering
#Launch the following command and compare with what you see in the UI on the right
xp = mlflow.search_experiments(filter_string=f"name LIKE '/Shared/dbdemos/experiments/mlops%'", order_by=["last_update_time DESC"])[0]
print(xp.experiment_id)
...
#Sort the best run id (this notebook) using the results in df
df = mlflow.search_runs(filter_string="metrics.val_f1_score < 1")
run = df.sort_values(by=/*to FILL*/, ascending=False)
...
# MAGIC %md
# MAGIC ### LAB: numerical columns
# MAGIC
# MAGIC Missing values for numerical columns are imputed with mean by default.
# MAGIC Use the cell above as inspiration to build also a pipeline that will be applied to the following fields: "monthly_charges", "total_charges", "tenure", "num_optional_services"
# MAGIC
# MAGIC The different steps are:
# MAGIC 1. Called converter: convert the values to numeric with errors set at 'coerce'
# MAGIC 2. Called imputers: The input for ColumnTransformer is num_imputers
# MAGIC 3. Called standardizer: We use a StandardScaler() function
# MAGIC
# MAGIC You can use another notebook automatically generated from the Experiments UI to get some inspirations
...
numerical_pipeline = Pipeline(steps=[
    ("converter", /*TO FILL*/,
    ("imputers", /*TO FILL*/,
    ("standardizer", /*TO FILL*/,
])

numerical_transformers = [("numerical", /*TO FILL*/)]
...
# MAGIC
# MAGIC LAB: use http://hyperopt.github.io/hyperopt/scaleout/spark/ to fill the next
# MAGIC You can also have more information about the different parameters in this [document](https://docs.databricks.com/en/machine-learning/automl-hyperparam-tuning/hyperopt-concepts.html)

# COMMAND ----------

# DBTITLE 1,Here it can be self done
trials = Trials()
fmin(/*TO FILL*/,
     space=/*TO FILL*/,
     algo=/*TO FILL*/,
     max_evals=/*TO FILL*/,  # Increase this when widening the hyperparameter search space.
     trials=/*TO FILL*/)
...
# DBTITLE 1,Do we need that part?
if shap_enabled:
